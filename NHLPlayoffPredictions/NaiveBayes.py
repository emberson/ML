...
if USE_RANDOM_FOREST_RANK: # Use variable importance from optimized random forest
    rf = RandomForestClassifier(n_estimators=100, max_depth=3, min_samples_split=2, min_samples_leaf=1, \
                                criterion="gini", max_features="auto", bootstrap=True, random_state=299792458, n_jobs=-1)
...
for i in range(nfeatures):
    ...
    result = cross_validate(nb, data.x_train, y=data.y_train, cv=kfold, scoring="accuracy", return_train_score=True)
...
nb.fit(data.x_train, data.y_train)
print("Test accuracy       : {:.2f}".format(nb.score(data.x_test, data.y_test)))
